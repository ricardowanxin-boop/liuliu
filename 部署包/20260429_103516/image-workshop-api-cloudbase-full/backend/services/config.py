...
DEFAULT_ZENMUX_MODEL = "bytedance/doubao-seedream-5.0-lite"
...
ZENMUX_IMAGE_EDIT_BLOCKLIST = {
    "openai/gpt-image-2": (
        "ZenMux openai/gpt-image-2 已验证文字生图可用，但图片编辑接口当前返回 "
        "Resource not found。请切换到支持图生图的模型或通道。"
    ),
    "google/gemini-2.5-flash-image": (
        "ZenMux google/gemini-2.5-flash-image 已验证接口会响应，但图生图请求未返回图片结果。"
        "请切换到已确认可交付的图生图通道。"
    ),
}
...
def build_public_config() -> dict[str, Any]:
    ...
    if default_provider == DOUBAO_PROVIDER:
        ...
        default_model = resolve_zenmux_model()
        if get_image_edit_block_reason(ZENMUX_PROVIDER, default_model):
            default_model = first_editable_zenmux_model()
    ...
    return {
        ...
        "modelCapabilities": build_model_capabilities(),
        "defaults": {
...
def build_model_capabilities() -> dict[str, dict[str, dict[str, Any]]]:
    """Return frontend-safe per-model capability hints."""
    zenmux_capabilities: dict[str, dict[str, Any]] = {}
    for item in ZENMUX_IMAGE_MODELS:
        slug = str(item["slug"])
        disabled_reason = get_image_edit_block_reason(ZENMUX_PROVIDER, slug)
        zenmux_capabilities[slug] = {
            "apiMode": item.get("api_mode", "auto"),
            "imageEditEnabled": disabled_reason is None,
            "imageEditNote": disabled_reason,
        }

    return {
        ZENMUX_PROVIDER: zenmux_capabilities,
        DOUBAO_PROVIDER: {
            str(item["slug"]): {
                "apiMode": "native",
            ...
            for item in DOUBAO_IMAGE_MODELS
        },
        OPENAI_PROVIDER: {
...
def first_editable_zenmux_model() -> str:
    """Return the first ZenMux model not known to be blocked for image editing."""
    if get_image_edit_block_reason(ZENMUX_PROVIDER, DEFAULT_ZENMUX_MODEL) is None:
        return DEFAULT_ZENMUX_MODEL
    for item in ZENMUX_IMAGE_MODELS:
        slug = str(item["slug"])
        if get_image_edit_block_reason(ZENMUX_PROVIDER, slug) is None:
            return slug
    return DEFAULT_ZENMUX_MODEL


def get_image_edit_block_reason(provider_type: str | None, model: str | None) -> str | None:
    """Return a known provider/model image-edit blocker, if any."""
    provider = normalize_provider_type(provider_type)
    selected_model = (model or "").strip()
    if provider == ZENMUX_PROVIDER and selected_model in ZENMUX_IMAGE_EDIT_BLOCKLIST:
        if os.getenv("ZENMUX_ALLOW_UNVERIFIED_IMAGE_EDIT", "").strip() == "1":
            return None
        return ZENMUX_IMAGE_EDIT_BLOCKLIST[selected_model]
    return None
